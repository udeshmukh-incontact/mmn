...
namespace InContact.Common.Branding
{
    public class PartnerBrandingServiceHelper : IPartnerBrandingServiceHelper
    {
        ...
        {
            var PartnerBrandingsApiError = JsonConvert.DeserializeObject<Error>(actualMessage);
            if (PartnerBrandingsApiError.Code == "404")
            {
                NotFoundException ex = new NotFoundException(PartnerBrandingsApiError.Message);
                ex.Data.Add("type", "notfound");
                throw ex;
            }
            else if (PartnerBrandingsApiError.Code == "503")
            {
                ServiceException ex = new ServiceException(PartnerBrandingsApiError.Message);
                ex.Data.Add("type", "servicedown");
                throw ex;
            }
            else if (PartnerBrandingsApiError.Code == "422")
                throw new FormatException(PartnerBrandingsApiError.Message);
            else
            {
                string errorMessage = "PartnerBrandingsApi returned status code:"+ PartnerBrandingsApiError.Code+','+" Message:" + PartnerBrandingsApiError.Message;
